...
from common_functions import bot, show_welcome_message, go_home, create_main_menu, is_admin, start_gemini_chat, handle_gemini_message, user_gemini_states
    ...
    try:
        ...
        if lecture[3]:
            markup.add(types.InlineKeyboardButton(
                "🎧 Аудио",
                callback_data=f"get_file_audio_{course}_{topic.replace(' ', '~')}"
            ))
        if lecture[4]:
            markup.add(types.InlineKeyboardButton(
                "📄 Документ",
                callback_data=f"get_file_document_{course}_{topic.replace(' ', '~')}"
            ))
        if lecture[5]:
            markup.add(types.InlineKeyboardButton(
                "📊 Презентация",
                callback_data=f"get_file_presentation_{course}_{topic.replace(' ', '~')}"
            ))
        if lecture[6]:  # photo_file_id
            markup.add(types.InlineKeyboardButton(
                "🖼 Фото",
                callback_data=f"view_photo_{course}_{topic.replace(' ', '~')}"
            ))
...
def handle_delete_lecture(call):
    ...
    try:
        ...
        match = re.match(r"delete_lecture_(\d+)_(.+)", call.data)
        if not match:
            logging.warning(f"Неверный callback delete_lecture: {call.data}")
            bot.answer_callback_query(call.id, "❌ Ошибка данных.")
            return
